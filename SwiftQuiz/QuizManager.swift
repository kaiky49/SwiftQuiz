...
class QuizManager {
  ...
  private var _totalCorrecteAnswers = 0
  
  var question: String {
    return quiz.question
  }
  var opitions: [String] {
    return quiz.opitions
  }
  var totalAnswers: Int {
    return _totalAnswers
  }
  var totalCorrectAnswers: Int {
    return _totalCorrecteAnswers
  }
  
  func refreshQuiz() {
    let randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))
    let quizData = quizes[randomIndex]
    quiz = Quiz(question: quizData.question, opitions: quizData.options, correctedAnswer: quizData.correctAnswer)
  }
  
  func validateAnswer(index: Int) {
    _totalAnswers += 1
    if quiz.validateOpition(index) {
     _totalCorrecteAnswers += 1
    }
  }
